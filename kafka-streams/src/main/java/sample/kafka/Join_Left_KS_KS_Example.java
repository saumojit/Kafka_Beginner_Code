...
public class Join_Left_KS_KS_Example {
    ...
    static void processorFunctionLogic(final StreamsBuilder builder) {
        ...
        JoinWindows joinwindow_of_5s = JoinWindows.ofTimeDifferenceWithNoGrace(Duration.ofSeconds(5)); // unlike ktable , for kstream_to_stream we have joining window concept (with or w/o grace)

        // specifying internal-kafka topic name as well
        StreamJoined joinedParams = StreamJoined.with(Serdes.String() ,Serdes.String() , Serdes.String() )
                                                .withName("joined-records") //( optional )
                                                .withStoreName("joined-records"); // ( optional )

        // TWO JOINS leftJoin AND outerJoin
        final KStream<String, JoinedRecord> stream_joined = stream1.leftJoin(stream2 , valueJoiner , joinwindow_of_5s , joinedParams).peek((key, value) -> System.out.println("O-put Key and Value Input => " +  key + " : "+ value));
...
// Streams =>   ./gradlew :kafka-streams:run -Dclass='Join_Left_KS_KS_Example'
...
// This is left join , meaning left data will be retained regardless of key matching
// another example : outerjoin ( implementation is same like left join )
